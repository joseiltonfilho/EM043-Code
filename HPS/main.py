...
coral_pixel = coral(name="coral", real=False)
...
def process_image():
    ...
    start = time.time()
    img = cv2.cvtColor(cv2.imread('coral_original.jpg'), cv2.COLOR_BGR2RGB)
    ...
    pixel_count = img[img > 0]
    pixel_len=len(pixel_count)
    return pixel_len
    ...
    try:
        ...
        while True:
            ...
            if now.hour == int(now.hour):
                ...
                ser.write(b"S-0")
                # Process image
                pixel_count_val=process_image()
                # Connect the client.
                await client.connect()
                # Configuring FPGA for cloud.
                if(hostname in 'de10nano'):
                    ...
                    if ( bridges[0] is not None ) : 
                        ...
                        watchdog_task = loop.create_task(watchdog_nios(control_bridge, 1))
                ...
                # Measure sensors and send data to cloud
                print(f'Sending telemetry from the provisioned device')
                gs_data = gs.get_telemetries()
                rfs_data = rfs.get_telemetries(data_bridge)
                coral_data=coral_pixel.get_telemetries(pixel_count_val)
                msg = coral_pixel.create_component_telemetry(coral_data)
                ...
                # Stop NIOS watchdog task
                if(watchdog_task is not None) :
                    watchdog_task.cancel()
                # Disconnect
                await client.disconnect()
                # send command to release BMCs for 30min
                ser.write(b"B-30")
